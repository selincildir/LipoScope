...
LipoScope is an open, browser-based lipoedema research atlas for literature exploration, PubMed surveillance, diagnostic standardization, standardized cohort building, gene and biomarker evidence mapping, gene literature checking, cohort design-gap prioritization, and minimum dataset export.
...
- Keeps user-added records, hidden-paper choices, profile labels, notes, cohort drafts, gene literature queries, and optional local PDF figure previews in browser storage on the user's own device unless the user chooses to export or share a Browse snapshot.
- Provides a Research Atlas with Corpus Status, Knowledge Graph, Candidate Gene and Biomarker Prioritizer, Standardized Cohort Builder, Gene and Literature Evidence Checker, Phenotype and Differential Diagnosis Matrix, Cohort Design Gap Radar, and Lipoedema Minimum Dataset Builder.
...
- Checks user-entered gene symbols or aliases against the PubMed-wide lipoedema evidence layer and predefined lipoedema-relevant biology or overlap-context programs for conservative research triage.
...
User-added papers, hidden-paper choices, profile labels, local notes, cohort drafts, gene literature queries, and optional local PDF figure previews are stored in the user's own browser storage. Different users on different devices have independent local collections unless they deliberately open the same shared Browse link.
...
- browser-local phenotype, cohort-rule, gene literature query, note, and figure-PDF inputs supplied by the user.
